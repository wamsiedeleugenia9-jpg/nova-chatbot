...
export default function Blueprint() {
  ...
  return <main style={shell}><div style={card}>
    ...
    {!state.started ? <><h1 style={{ fontSize: 48 }}>{content.title}</h1><p style={{ color: "#ddd6fe", lineHeight: 1.8 }}>{content.introduction}</p><button disabled={busy} onClick={() => act({ action: "start" })} style={button}>Încep Creator Blueprint</button></> : <>
      <div style={{ marginTop: 24, color: "#c4b5fd" }}>Atelierul {atelier.number} din 7 · {Math.round(((atelier.number - 1) / 7) * 100)}% parcurs</div>
      ...
      {!allAnswered && <section style={{ marginTop: 24, padding: 24, borderRadius: 20, background: "rgba(109,40,217,.12)", border: "1px solid rgba(167,139,250,.2)" }}>
      ...
      {allAnswered && !state.summary && <section style={{ marginTop: 24, padding: 24, borderRadius: 20, background: "rgba(109,40,217,.12)", border: "1px solid rgba(167,139,250,.2)" }}><p>Răspunsurile sunt salvate. Rezumatul atelierului nu a putut fi generat încă.</p><button disabled={busy} onClick={() => act({ action: "summarize" })} style={button}>{busy ? "EWA construiește rezumatul…" : "Încearcă din nou rezumatul"}</button></section>}
      {state.summary && <section style={{ marginTop: 24, padding: 24, borderRadius: 20, background: "rgba(219,39,119,.08)", border: "1px solid rgba(244,114,182,.25)" }}><p style={{ color: "#f0abfc" }}>{atelier.summaryLead}</p><div style={{ whiteSpace: "pre-wrap", lineHeight: 1.8, fontSize: 18 }}>{state.summary}</div><p style={{ color: "#c4b5fd", lineHeight: 1.6 }}>{content.validation}</p>
        {!state.completed && !adjusting && <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button disabled={busy} onClick={() => act({ action: "confirm" })} style={button}>Confirm, continuăm</button><button onClick={() => setAdjusting(true)} style={secondary}>Ajustăm ceva</button><button disabled={busy} onClick={() => act({ action: "pause" })} style={secondary}>Pauză, revin mai târziu</button></div>}
        ...
        {state.completed && <div><p style={{ color: "#6ee7b7" }}>✓ Atelier confirmat și salvat.</p><p style={{ lineHeight: 1.7 }}>{atelier.next}</p>{atelier.number < 7 ? <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button disabled={busy} onClick={() => act({ action: "continue" })} style={button}>Continuăm acum</button><button disabled={busy} onClick={() => act({ action: "pause" })} style={secondary}>Pauză, revin mai târziu</button></div> : <p style={{ color: "#fde68a" }}>Atelierele 1–7 sunt complete. Atelierul 8 — Creator DNA urmează în Etapa 4.</p>}</div>}
      </section>}
    </>}
