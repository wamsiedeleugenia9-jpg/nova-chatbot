...
import { blueprintLoadPhase } from "../lib/blueprint/state";
...
export default function Blueprint() {
  ...
  async function loadBlueprint() {
    setError("");
    try { update(await request(session, "GET")); }
  ...
  const loadPhase = blueprintLoadPhase(session, content, error);
  if (loadPhase === "authenticating" || loadPhase === "loading") return <main style={shell}><div style={card}>Se încarcă…</div></main>;
  if (loadPhase === "error") return (
    <main style={shell}><div style={{ ...card, textAlign: "center" }}>
      ...
      <button onClick={loadBlueprint} style={button}>Încearcă din nou</button>
      <div><a href="/" style={{ display: "inline-block", color: "#a78bfa", marginTop: 22 }}>← Înapoi la EWA AI</a></div>
    </div></main>
  );
  if (loadPhase === "unauthenticated") return (
    <main style={shell}><div style={{ ...card, textAlign: "center" }}>
      <h1>Creator Blueprint</h1>
