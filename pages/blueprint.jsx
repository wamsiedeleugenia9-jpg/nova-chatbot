...
export default function Blueprint() {
  ...
  return <main style={shell}><div style={card}>
    ...
    {data.creatorDna ? <><h1 style={{ fontSize: 42 }}>Creator DNA</h1><p style={{ color: "#6ee7b7" }}>✓ Creator Blueprint finalizat</p>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 32 }}><button onClick={copyDna} style={button}>Copy text</button><button onClick={downloadDna} style={secondary}>Download</button>{!state.blueprintCompleted && <button disabled={busy} onClick={() => act({ action: "generate_dna" })} style={secondary}>Finalizează starea</button>}</div>
      ...
      {state.summary && <section style={{ marginTop: 24, padding: 24, borderRadius: 20, background: "rgba(219,39,119,.08)", border: "1px solid rgba(244,114,182,.25)" }}><p style={{ color: "#f0abfc" }}>{atelier.summaryLead}</p><div style={{ whiteSpace: "pre-wrap", lineHeight: 1.8, fontSize: 18 }}>{state.summary}</div><p style={{ color: "#c4b5fd", lineHeight: 1.6 }}>{content.validation}</p>
        {!state.completed && !adjusting && <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button disabled={busy} onClick={() => act({ action: "confirm" })} style={button}>Confirm, continuăm</button>{atelier.number < 8 && <button onClick={() => setAdjusting(true)} style={secondary}>Ajustăm ceva</button>}<button disabled={busy} onClick={() => act({ action: "pause" })} style={secondary}>Pauză, revin mai târziu</button></div>}
