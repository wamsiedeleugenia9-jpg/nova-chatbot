  ...
  try {
    ...
    if (action === "start") {
      ...
      for (const item of changed) {
        ...
        preparedChanges.push({ questionNumber: item.questionNumber, rawAnswer: item.rawAnswer, interpretation });
      }
      ...
      const latestAnswers = records.answers.map(item => {
        const replacement = submitted.find(answer => item.atelier_number === atelierNumber && answer.questionNumber === item.question_number);
        return replacement ? { ...item, raw_answer: replacement.rawAnswer } : item;
      });
      const latestSections = records.sections.map(item => item.atelier_number === atelierNumber
        ? { ...item, interpreted_summary: summary.summary, key_elements: summary.keyElements }
        : item);
      const why = latestAnswers.find(item => item.atelier_number === 8 && item.question_number === 1)?.raw_answer;
      const generated = await askCreatorDna(creatorDnaPrompt({ sections: latestSections.filter(item => item.atelier_number <= 7), answers: latestAnswers.filter(item => item.atelier_number <= 7) }));
      const transaction = await client.rpc("save_blueprint_workshop_edit", {
        p_atelier_number: atelierNumber,
        p_changed_answers: preparedChanges,
        p_summary: summary.summary,
        p_key_elements: summary.keyElements,
        p_creator_dna: appendWhy(generated, why)
      });
    ...
    return res.status(200).json({ ...responsePayload(records), paused: action === "pause", saved: action === "save_edit" });
  } catch (error) {
