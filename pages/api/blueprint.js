...
async function load(client, userId) {
  const [blueprint, sections, answers] = await Promise.all([
  ...
  const error = blueprint.error || sections.error || answers.error;
  if (error) throw error;
  return { blueprint: blueprint.data, sections: sections.data || [], answers: answers.data || [] };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") return res.status(200).json({ content: CONTENT, state: blueprintState(records.blueprint, records.sections, records.answers) });
    ...
    if (action === "start") {
      ...
      if (questionNumber === atelier.questions.length) {
        ...
        const summary = await askClaude(sectionSummaryPrompt({ atelier, answers: completeAnswers }), true);
