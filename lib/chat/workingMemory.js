...
const extractionSchema = {
  ...
  required: ["remember", "category", "content", "project_key"],
  properties: {
...
async function saveExtractedMemory(client, userId, extraction, existingMemories = []) {
  const memory = validateExtraction(extraction);
  if (!memory || !memory.remember) return { action: "none" };

  const duplicate = existingMemories.find(item =>
    item.category === memory.category &&
    (item.project_key || null) === memory.project_key &&
    isNearDuplicate(item.content, memory.content)
  );
...
function memoryExtractionRequest(userMessage, assistantReply) {
  return {
    ...
    max_tokens: 350,
    system: `Extrage cel mult o singură memorie operațională utilă pentru o conversație viitoare. Fii conservator. Reține numai proiecte active, decizii finale de conținut, planuri temporare, următorul pas sau un workflow explicit. Nu reține saluturi, glume, emoții temporare, explorări, variante respinse, conversație generală sau fapte strategice de Creator DNA. Nu propune modificări Creator DNA. Dacă nimic nu merită reținut, setează remember=false, content="", project_key=null. Conținutul trebuie să fie concis, autonom și în română.`,
    messages: [{
      role: "user",
