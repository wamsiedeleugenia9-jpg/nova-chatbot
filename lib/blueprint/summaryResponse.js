...
function summaryFromResponse(content) {
  ...
  const keyElements = input?.keyElements ?? input?.key_elements;
  if (typeof input?.summary !== "string" || !input.summary.trim() || !Array.isArray(keyElements) || keyElements.some(item => typeof item !== "string")) {
    throw new Error("Anthropic returned an invalid structured summary");
  }
  return { summary: input.summary.trim(), keyElements: keyElements.map(item => item.trim()).filter(Boolean) };
}
