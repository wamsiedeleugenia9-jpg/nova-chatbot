...
test("EWA treats supplied cross-session history as continuity without inventing unavailable memory", () => {
  const api = readFileSync(join(__dirname, "..", "pages", "api", "chat.js"), "utf8");
  assert.match(api, /istoricul conversatiei pot proveni din sesiuni autentificate anterioare ale aceluiasi utilizator/);
  assert.match(api, /nu afirma ca nu ai acces la sesiunile anterioare atunci cand raspunsul cerut este disponibil acolo/);
  assert.match(api, /nu o ai in contextul disponibil si nu o inventa/);
  assert.match(api, /Nu sugera acces la conversatiile altor utilizatori sau la informatii din afara contextului furnizat de aplicatie/);
});
